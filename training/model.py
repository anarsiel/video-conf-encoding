...
def create_model(save_plot=False):
    ...
    # x = (im_inputs - keras.backend.mean(im_inputs, axis=(0, 1, 2), keepdims=2)) / keras.backend.std(im_inputs, axis=(0, 1, 2), keepdims=2)

    x = lr.Conv2D(16, (3, 3), padding='same')(im_inputs)
    x = lr.Activation(ACT)(x)
    x = lr.BatchNormalization()(x)

    x = lr.Conv2D(16, (3, 3), padding='same')(x)
    x = lr.BatchNormalization()(x)
    ...
    z = lr.BatchNormalization()(z)
    z = lr.Conv3DTranspose(1, 2, padding='valid')(z)
    z = lr.BatchNormalization()(z)
    z = lr.TimeDistributed(lr.Conv2D(32, 3, padding='same'))(z)
    z = lr.BatchNormalization()(z)
    z = lr.Conv3D(32, 3, padding='same')(z)
    z = lr.BatchNormalization()(z)
    z = lr.ConvLSTM2D(64, 3, padding='same', return_sequences=True)(z)
    z = lr.BatchNormalization()(z)
    ...
    # x = lr.ConvLSTM2D(128, 3, padding='same', return_sequences=True)(x)

    x = lr.TimeDistributed(lr.Conv2DTranspose(128, 3, strides=(2, 2), padding='same'))(x)
    x = lr.Activation(ACT)(x)
    x = lr.BatchNormalization()(x)
    ...
    x = lr.TimeDistributed(lr.Conv2D(128, 3, padding='same'))(x)
    x = lr.Activation(ACT)(x)
    x = lr.BatchNormalization()(x)
    x = lr.TimeDistributed(lr.Conv2D(64, 3, padding='same'))(x)
    x = lr.Activation(ACT)(x)
    x = lr.BatchNormalization()(x)
    ...
    x = lr.Activation(ACT)(x)
    x = lr.BatchNormalization()(x)
    x = lr.ConvLSTM2D(3, 3, padding='same', return_sequences=True)(x)
    x = lr.Activation(ACT)(x)
    x = lr.BatchNormalization()(x)

    x = lr.TimeDistributed(lr.Conv2D(3, 2, padding='same'))(x)
    x = lr.Activation(ACT)(x)
    x = lr.BatchNormalization()(x)
    x = lr.TimeDistributed(lr.Conv2D(3, 2, padding='same'))(x)
    x = lr.Activation(ACT)(x)
